...
NeteaseMusicMetadata::NeteaseMusicMetadata(Json::Value raw) {
	mName = raw["musicName"].asString();
	mAlbum = raw["album"].asString();
	for (int i = 0; i < raw["artist"].size(); i += 1) {
		if (!mArtist.empty()) mArtist += "/";
		mArtist += raw["artist"][i][0].asString();
	}
	mBitrate = raw["bitrate"].asInt();
	mDuration = raw["duration"].asInt();
	mFormat = raw["format"].asString();
}

bool NeteaseCrypt::openFile(std::string const& path) {
	mFile.open(std::filesystem::path(path), std::ios::in | std::ios::binary);
...
void NeteaseCrypt::buildKeyBox(unsigned char* key, int keyLen) {
	for (int i = 0; i < 256; ++i) mKeyBox[i] = (unsigned char)i;
...
int NeteaseCrypt::Dump(std::string const& outputDir, size_t& fileSize) {
	size_t filesize = 0;
	if (outputDir.empty()) mDumpFilepath = std::filesystem::path(mFilepath);
	else mDumpFilepath = std::filesystem::path(outputDir) / std::filesystem::path(mFilepath).filename();
	...
	while (!mFile.eof()) {
		...
		if (!output.is_open()) {
			// identify format
			// ID3 format mp3
			if (buffer[0] == 0x49 && buffer[1] == 0x44 && buffer[2] == 0x33) mFormat = NeteaseCrypt::MP3;
			else mFormat = NeteaseCrypt::FLAC;
...
NeteaseCrypt::NeteaseCrypt(std::string const& path) {
	...
	for (size_t i = 0; i < n; i += 1) keydata[i] ^= 0x64;
	...
	if (n <= 0) {
		...
		reader.parse(modifyDecryptData, test);

		//std::cout << test.toStyledString() << std::endl;

		mMetaData = new NeteaseMusicMetadata(test);
	}
	...
	else std::cout << "[Warn] '" << path << "' missing album can't fix album image!" << std::endl;
